...
### ✅ Controller Layer (`src/controller/`) - UPDATED

| File | Status | Description |
|------|--------|-------------|
| `ApplicationController.h/cpp` | ✅ Updated | Now uses Model layer, all adjustment slots |
| `ICommand.h` | ✅ Complete | Command pattern interface |
| `Commands.h` | ✅ Complete | Command implementations |

### ✅ UI Layer (`src/ui/`) - WORKING
...
### ✅ Image Processing (`src/image_processing/`) - COMPLETE
...
```bash
git add -A
git commit -m "feat: implement MVC pattern with Model layer"
```
...
├── model/                      # NEW: MVC Model layer
│   ├── ImageDocument.h/cpp
│   ├── AdjustmentSettings.h/cpp
│   ├── AppState.h/cpp
...
│   ├── core/
│   │   ├── image_pipeline.h/cpp
│   │   ├── operation_base.h
│   │   └── operation_registry.h/cpp
│   ├── operations/
│   │   ├── light/
│   │   ├── color/
│   │   ├── detail/
...
│   ├── utils/
│   └── CMakeLists.txt
├── raw_processing/             # RAW file support
└── document/                   # Legacy (to be removed)
```
